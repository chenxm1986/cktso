...
int main()
{
    ...
    long long ai[13] = { 0, 3, 4, 1, 4, 1, 2, 3, 2, 4, 0, 3, 5 };
    long long ap[7] = { 0, 3, 5, 7, 8, 10, 13 };
    double b[6] = { 35.95, 53.9, 7.7, -17.6, 60.83, 98.18 };
    double x[6];
    int iter = 5;

    //Create solver instance
    ICktSo_L instance = nullptr;
    int *iparm;
    const long long *oparm;
    ret = CKTSO_L_CreateSolver(&instance, &iparm, &oparm);
    if (ret < 0)
    {
    ...
    for (int j = 0; j < iter; ++j)
    {
        ...
        for (long long i = 0; i < nnz; ++i) ax[i] *= (double)rand() / RAND_MAX * 2.;
        ...
        {
            double s = 0.;
            const long long start = ap[r];
            const long long end = ap[r + 1];
            for (long long c = start; c < end; ++c)
            {
                s += ax[c] * x[ai[c]];
            }
            s -= b[r];
